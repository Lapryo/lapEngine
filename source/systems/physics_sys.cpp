...
void PhysicsSystem::Update(float deltaTime, entt::registry &registry)
{
    ...
    for (auto [entity, origin, physics] : view.each())
    {
        ...
        rl::Vector2 oldPos = origin.position;
        rl::Vector2 newPos = {origin.position.x + physics.velocity.x * deltaTime - physics.gravity.x * deltaTime,
        ...
        if (physics.collidable)
        {
            ...
            for (auto [e2, o2, p2] : view.each())
            {
                if (e2 != entity && p2.collidable)
                {
                    ...
                    if (rl::CheckCollisionRecs(thisRect, otherRect))
                    {
                        // get the direction between the old position and the new position
                        rl::Vector2 difference = {newPos.x - oldPos.x, newPos.y - oldPos.y};
                        ...
                        while (rl::CheckCollisionRecs(thisRect, otherRect))
                        {
